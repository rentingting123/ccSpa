<template>
  <div class="competitionExpo">
      <div class="bannerbox">
          <a class="bannerItem"><img src="@/mnskin/assets/img/banner1.png" alt=""></a>
      </div>
      <div>
          <div class="headline">展会介绍</div>
          <div class="intruce">
              <p>中国高等教育博览会（HIGHER EDUCATION EXPOCHINA，简称高博会）是由教育部批准，中国高等教育学会主办的国际领先的集现代教育高端装备展示、高等教育学术交流、教学改革成果推介、教师专业化发展培训、科研成果转化、科技创新企业孵化、技术服务、贸易洽谈等为一体的高品质、综合性、专业化的著名品牌活动。</p><br><br>
              <template v-if="showMore">
                  <p>为落实“教育这十年”相关工作安排，充分展示党的十八大以来，我国高等教育事业取得的历史性成就，经教育部批准，第58届中国高等教育博览会将于2022年12月2-4日在重庆市举办。</p><br>
                  <p>本届高博会以“校地聚合·产教融合：高质量发展”为主题，将首次设立<span
                          style="color:#f00">竞赛专区</span>，计划邀请全国150个竞赛组委会参展，集中开展线下活动。<b>展示竞赛在创新人才培养上的成果和特色，助力竞赛资源拓展、宣传推广、产教融合、成果与需求信息发布等，</b>加强沟通交流，引导竞赛工作良性开展，促进竞赛高质量发展。
                  </p>
              </template>
              <div class="moreBtn" @click="moreLook">{{showMore?'收起':'查看更多>>'}}</div>
          </div>
      </div>
      <div>
          <div class="headline">会展权益</div>
          <ul class="competitionMain">
              <div class="equityIist">
                  <li v-for="(item,i) in equityList" :key="item.key" class="equityItem">
                      <div class="mess" @click="changeTypes(i)"
          ...
          <div class="areabox">
              <a class="area">
                  <div class="text1">
                      <img src="@/mnskin/assets/img/logo_heec.png" alt="">
                      第58届<br>中国高等教育博览会【竞赛专区】
                  </div>
                  <div class="text2">12月2日-4日 中国·重庆</div>
                  <div class="text3">立即参展</div>
      ...
      <div>
          ...
          <div class="exhAreaTab">
              <div class="exhAreaItem" v-for="(item,index) in exhAreaList" :key="index">
                  <div :class="{active:current == item.id}" class="tab" @click="changeTabs(item.id)"><img
                          :src="item.icon" alt="">{{item.name}}</div>
              </div>
          </div>
          <div class="exhAreaList">
              <div class="exhAreaContent" v-for="(item,index) in exhAreaList" :key="index">
                  <template v-if="item.id==current">
                      <div class="left">
                      ...
                      <div class="right">
                          ...
                          <ul>
                              <li v-for="(list,index) in item.details" :key="index">{{index+1}}.{{list}}</li>
                          </ul>
                      </div>
                  </template>
              </div>
      ...
      <div class="competitions">
          ...
          <div class="competitionsList">
              <div class="part" v-for="(item,index) in competitions" :key="index">
...
export default {
  data() {
      return {
          ...
          equityList: [
              {
                  key: 1,
                  name: '品<br>牌<br>曝<br>光',
                  background: require("@/nskin/assets/img/equity4.png"),
                  checked: false,
                  content: '全媒体矩阵宣传，展厅主屏播放宣传视频，竞赛名片线上展示，优先承办相关论坛。',
              },
              {
                  key: 2,
          ...
          exhAreaList: [
              {
                  id: 1,
                  icon: require('@/mnskin/assets/img/icon1.png'),
                  name: '发展历程与研究成果专区',
                  title: '展示创新教育10年发展历程以及展现“创新人才培养这十年”的成果和风采。',
                  pic: require('@/mnskin/assets/img/introduce1.png'),
                  details: [
                      '创新教育（学科竞赛）10年历程概述',
                      '专家工作组研究成果（以竞赛质量提升为导向）',
                      '高校学生竞赛与教师发展数据平台展示'
                  ],
              },
              {
                  id: 2,
                  icon: require('@/mnskin/assets/img/icon2.png'),
                  name: '高质量竞赛专区',
                  title: '展示竞赛在创新人才培养上的成果和特色，助力竞赛资源拓展、宣传推广、科技成果转化、产教深度融合、成果与需求信息发布等，促进竞赛高质量发展。',
                  pic: require('@/mnskin/assets/img/introduce2.png'),
                  details: [
                      '榜单内56个竞赛展区',
                      '全国高校教师教学创新大赛',
                      '全国高校青年教师教学竞赛',
              ...
              {
                  id: 3,
                  icon: require('@/mnskin/assets/img/icon3.png'),
                  name: '交流互动区',
                  title: '参展竞赛可围绕赛事相关主题，优先合作，组织开展竞赛相关论坛、发布会、闭门会、决赛竞赛路演等会议活动。',
                  pic: require('@/mnskin/assets/img/introduce3.png'),
                  details: [
                      '指数发布：高校教师教学发展指数发布',
                      '竞赛发布：竞赛通知、决赛路演、现场颁奖',
                      '竞赛路演：竞赛推介、作品路演、成果展示',
                      '交流对洽：专家报告、闭门交流会、主题研讨会'
                  ],
              },
          ],
          competitions: [
  ...
  methods: {
      ...
      changeTypes(i) {
          ...
          this.content = this.equityList[i].content
      },
      changeTabs(id) {
          this.current = id
      },
