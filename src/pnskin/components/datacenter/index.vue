<template>
    <div>
        <!-- 导航栏 -->
        <navs :lists="lists" :homeUrl="homeUrl"/>
...
<script>
import navs from "@/npskin/view/datacenter/navs";
...
export default {
    ...
    created(){
      let info = getWebInfo();
      this.lists = info && info.navsList || this.listss
      this.$set(this.lists[6],'show',false)
      this.homeUrl = info && info.homeUrl
    },
    data(){
        return{
          homeUrl: '',
...
.view{
    min-height: calc(100vh - 300px - 38px - 37px);
}
</style>