<template>
  <div class="works" v-if="lists.length>0">
    <h2 class="title">
      <p>历届作品</p>
      <span @click="$router.push('/datacenter/half/previousworks')">more</span>
    </h2>
    <div id="demo5" @mouseover="clearinter" @mouseleave="start">
      <ul id="demo6">
...
export default {
  ...
  methods: {
    start(rep) {
      ...
      let lengths = document.getElementById("demo6").offsetWidth;
      let showlengths = document.getElementById("demo5").offsetWidth;

      if (lengths < 2 * showlengths && lengths > showlengths)
        this.listX = this.listX.concat(this.lists);
      else if (lengths < showlengths) return;
      if (rep == "rep")
        document.getElementById("demo6").style.left =
          "-" + (lengths / 2 - showlengths) + "px";

      clearInterval(this.setinter);
      this.setinter = setInterval(() => {
        let left = document.getElementById("demo6").offsetLeft;
        if (lengths - showlengths <= Math.abs(left)) {
          this.start("rep");
        } else {
          document.getElementById("demo6").style.left = left - 1 + "px";
        }
      }, 30);
    },
    clearinter() {
