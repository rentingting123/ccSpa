<template>
    <el-dialog class="invoice" title="线下支付数据" :append-to-body="true" :modal-append-to-body="false" :close-on-click-modal=false :visible.sync="outlineDia"  width="70%">
        <el-form label-position="top" ref="form1" :model="form1" class="form">
            ...
            <el-form-item label="收费金额" prop="payrule">
                <el-input v-model="form1.rules" disabled></el-input>
            </el-form-item>
            <el-form-item label="线下付费凭证(请上传汇款回执单和团队名称信息截图)" prop="imgUrls" :rules="[{ required: true, message: '付费凭证不能为空', trigger: 'blur' }]">
                <upload
            ...
            <el-form-item label="备注" prop="remark" :rules="[{ required: true, message: '备注不能为空', trigger: 'blur' }]">
                <el-input type="textarea" :rows="3" v-model="form1.remark" placeholder=""></el-input>
            </el-form-item>
            <div class="paychoose">
...
export default {
    props:['close','payDetail','trainingId'],
    components:{
        upload
    },
    data(){
    ...
    async created(){
        ...
        this.form1 = {
            ...
            rules:rules,
            imgUrls:payDetail.imgUrls&&JSON.parse(payDetail.imgUrls)||[]
        }
    },
    methods:{
        ok(formName){
             this.$refs[formName].validate(async (valid) => {
                ...
                let obj = {
                    imgUrls:JSON.stringify(this.form1.imgUrls),
                    remark:this.form1.remark,
                    trainingId:this.trainingId
                }
                let result = await network.saveTrainingPay({data:obj});
        ...
        coverSuccess(file,filelist){
            this.form1.imgUrls.push({
                name:filelist.name,
                location:file.location
            })
        },
        delFileFun(j){
            this.form1.imgUrls.splice(j,1)
        },
    },
    watch:{
        outlineDia(val){
