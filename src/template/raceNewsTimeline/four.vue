  ...
  <div class="ContestDynamic">
    <div class="container">
      <div class="titleContent">
        <h3>{{$t('skin.newsType1')}}</h3>
        <p>INFORMATION</p>
      </div>
      
      <ul class="newsnotice">
        <li @click="choose(0)" :class="{'on':type==0}">{{$t('skin.newsType2')}}</li>
        <li @click="choose(1)" :class="{'on':type==1}">{{$t('skin.newsType3')}}</li>
        <li @click="choose(4)" :class="{'on':type==4}">{{$t('skin.newsType4')}}</li>
        <li @click="choose(5)" :class="{'on':type==5}">{{$t('skin.newsType5')}}</li>
      </ul>
...
export default {
  data(){
    return{
      news:[],
      type:'0',//0 大赛进程  1赛事动态  2章程细则 3组织机构
      page:1,
      pageSize:10,
      total:0
    }
