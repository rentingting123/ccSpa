    ...
    <el-dialog :visible.sync="dialogVisible" center :fullscreen="true" class="dialogVisible" :show-close="false" :close-on-press-escape="false" :close-on-click-modal="false" width="800px">
      ...
      <el-form ref="form" class="form" :model="form" label-width="120">
        <el-form-item 
          ...
          >
          <el-input @blur="change(table,index)" v-model="form[index+'@'+table.key]"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    async change(table,index){
      if(table.key=='phone'){
        let userId = this.form[index+'@'+table.key];
        if(!userId) return;
        ...
        this.$confirm('该手机号已报名，是否导出数据', '提示', {
    ...
    submit(formName){
      this.$refs[formName].validate(async (valid) => {
        ...
        Object.keys(this.form).forEach((key)=>{
          let keys = key.split("@");
          if(keys[1]=='phone') userId = this.form[key];
          arr.push({
        ...
        // let name = await jxapi.skinSet({data:{html:this.form,type:6}});
